...
def home():
    ...
    # COALESCE to handle NULL values for views, downloads, likes, and
    # resolution (just makes it say 0 rather than 'none')
    assets_data = query("""
                        SELECT id,name,COALESCE(views,0),COALESCE(downloads,0),
                        COALESCE(likes,0),COALESCE(resolution,'Unknown')
                        FROM asset
                        """)
...
def upload():
    # get request means loading the page normally
    # (not actually uploading anything)
    ...
    if not name:
        return render_template("404.html",
                               error="400 Bad Request: Name is required."), 400
    if len(name) > 20:
        return render_template("404.html",
                               error="""400 Bad Request: Name must be 20
                                characters or fewer."""), 400
    if len(description) > 1000:
        return render_template("404.html",
                               error="""400 Bad Request: Description must
                                be 1000 characters or fewer."""), 400
    ...
    # this little fella gets the resolution of the diffuse image
    # without BREAKING LIKE IT DID BEFORE
    resolution = "Unknown"
    ...
    for tag_id in selected_tags:
        query("INSERT INTO assetTags(Model_ID, Tag_ID) VALUES(?, ?)",
              (asset_id, tag_id),
              commit=True)
...
def asset(id):
    ...
    asset_rows = query("""SELECT id,name,views,downloads,likes,description
    ...
    query("UPDATE asset SET views = COALESCE(views, 0) + 1 WHERE ID = ?",
          (id,),
          commit=True)

    assettags = query("""SELECT name,icon
                      FROM Tag
                      WHERE ID IN
                      (SELECT Tag_ID FROM assetTags WHERE Model_ID = ?)""",
                      (id,))
    ...
    return render_template('asset.html',
                           show_navbar=False, asset=asset_rows,
                           assettags=assettags,
...
def download_image(asset_id, image_type):
    query("""UPDATE asset SET downloads =
          COALESCE(downloads, 0) + 1 WHERE ID = ?""",
          (asset_id,), commit=True)
    ...
    file_path = os.path.join(app.config["UPLOAD_FOLDER"],
                             f"{asset_id}{image_type}.png")
    if not os.path.exists(file_path):
        return "File not found", 404

    return send_file(file_path, as_attachment=True,
                     download_name=f"{asset_id}{image_type}.png")
